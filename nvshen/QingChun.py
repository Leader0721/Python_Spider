...
# 图片存放路径
FOLDER_SAVE = r'E:/Sprider/qingchun'
...
def download_image(image_title, image_detail_websites):
    ...
    for i in image_detail_websites:
        ...
        file_path = '%s/%s_%s.jpg' % (FOLDER_SAVE, image_title, num)  # 生成图片文件名
        if os.path.exists(file_path):  # 避免重复下载
            print('[exists]')
            return
        request.urlretrieve(i, file_path)
        # filename = '%s%s.jpg' % (image_title, num)
        print('正在下载图片：%s第%s/%s张，图片的地址是%s' % (image_title, num, amount, i))
